...
## Semantic JavaScript and CSS Directives

PTEGo provides semantic directives for embedding inline or block JavaScript and CSS assets into templates.

### 1. Semantic JavaScript Directives (`|js|`)

- **Inline Expression (`|js expr|`)**: Evaluates `expr` and wraps the output in `<script>...</script>`.
  ```pte
  |js "console.log('Template Initialized');"|
  ```
  Rendered output:
  ```html
  <script>console.log('Template Initialized');</script>
  ```

- **Block Directive (`|js|...|/js|`)**: Renders child template nodes inside a `<script>` tag.
  ```pte
  |js|
  const user = "|username|";
...
### 2. Semantic CSS Directives (`|css|`)

- **Inline Expression (`|css expr|`)**: Evaluates `expr` and wraps the output in `<style>...</style>`.
  ```pte
...
- **Block Directive (`|css|...|/css|`)**: Renders child template nodes inside a `<style>` tag.
  ```pte
  |css|
  .button {
...
PTEGo provides mechanisms for working with literal pipe characters (`|`).

### Escaped Literal Pipes (`\|`)

Precede a pipe character with a single backslash (`\|`) to output a literal pipe character:
